...
class TransporteServiciosProvider with ChangeNotifier{
  ...
  Future<void> fechtGuiasServicios(String ruc) async {
    ...
    notifyListeners();
    try{
      final response = await  http.get(Uri.parse('http://190.107.181.163:81/aqnq/ajax/lista_guias_servicio.php?ruc=$ruc'));
      if(response.statusCode == 200){
        final List<dynamic> data = jsonDecode(response.body);
        _guiasServicio = data.map((jsonServ) => Servicios.fromJson(jsonServ)).toList();
  ...
  void searchGuiaServicio(String query){
    if(query.isEmpty){
      ...
      _filteredGuiaServicio = _guiasServicio.where((guia) => (guia.guia).toLowerCase().contains(query.toLowerCase()) || 
